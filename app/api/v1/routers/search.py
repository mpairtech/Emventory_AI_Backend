...
async def _list_indexed_products(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to list indexed products.")
...
async def semantic_search(
    ...
    _quota: None = Depends(check_search_quota),     # ← QUOTA
    db: AsyncSession = Depends(get_db),
):
    try:
...
async def rag_search(
    request: SearchRequest,
    provider: str | None = Query(None, description="LLM provider: 'gemini' or 'openai'."),
    _auth:  None = Depends(verify_api_key),
    _quota: None = Depends(check_search_quota),     # ← QUOTA
    db: AsyncSession = Depends(get_db),
):
    if provider:
        ...
        if provider not in ("gemini", "openai"):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Provider must be 'gemini' or 'openai'.")
    ...
    logger.info("RAG search — query: '%s', org_id: %s, provider: %s", request.query, request.org_id, selected_provider)
    ...
    try:
        ...
        try:
            result = await SearchService.rag_search(
                ...
                llm_provider=fallback_provider, top_k=request.top_k, filters=request.filters,
            )
            return result
        except Exception as fallback_error:
            logger.error("Both providers failed: %s | %s", e, fallback_error, exc_info=True)
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="AI generation failed on all available providers.")
...
# ---------------------------------------------------------------------------
# /debug  (quota NOT applied — internal tool only)
# ---------------------------------------------------------------------------
